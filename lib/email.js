...
export async function sendAdminOrderNotificationEmail(
  ...
  try {
    ...
    const ADMIN_EMAIL = process.env.ADMIN_EMAIL || "farzstore02@gmail.com";
      ...
      .map((item) => {
        ...
        return `
          <tr style="border-bottom: 1px solid #E5E7EB;">
            <td style="padding: 10px 8px; font-size: 14px; color: #111827;">${name}</td>
            <td style="padding: 10px 8px; font-size: 14px; color: #6B7280; text-align: center;">${qty}</td>
            <td style="padding: 10px 8px; font-size: 14px; color: #6B7280; text-align: right;">₦${price.toLocaleString()}</td>
            <td style="padding: 10px 8px; font-size: 14px; font-weight: 600; color: #14532D; text-align: right;">₦${total.toLocaleString()}</td>
          </tr>
        `;
      })
      .join("");

    const adminDashboardUrl = `${process.env.NEXT_PUBLIC_APP_URL}/admin/orders`;

    const data = await resend.emails.send({
      from: FROM_EMAIL,
      to: ADMIN_EMAIL,
      subject: `💰 New Order Paid — ${order.order_number} (₦${totalAmount.toLocaleString()})`,
      html: `
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <tr>
      <td align="center">
        <table width="600" cellpadding="0" cellspacing="0" style="background-color: #FFFFFF; border-radius: 12px; overflow: hidden; border: 1px solid #E5E7EB;">
          ...
          <!-- Header -->
          <tr>
            <td style="background: #14532D; padding: 28px 32px; text-align: left;">
              <p style="margin: 0 0 4px 0; font-size: 12px; font-weight: 600; letter-spacing: 0.12em; text-transform: uppercase; color: #86EFAC;">
                Farz Supplements · Admin Alert
              </p>
          ...
          <!-- Key numbers bar -->
          <tr>
            <td style="background: #F0FDF4; border-bottom: 1px solid #BBF7D0; padding: 20px 32px;">
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td style="text-align: left;">
                    <p style="margin: 0 0 2px 0; font-size: 11px; color: #166534; font-weight: 600; text-transform: uppercase; letter-spacing: 0.1em;">Order</p>
                    <p style="margin: 0; font-size: 20px; font-weight: 700; color: #14532D;">${order.order_number}</p>
                  </td>
                  <td style="text-align: center;">
                    <p style="margin: 0 0 2px 0; font-size: 11px; color: #166534; font-weight: 600; text-transform: uppercase; letter-spacing: 0.1em;">Amount Paid</p>
                    <p style="margin: 0; font-size: 22px; font-weight: 700; color: #14532D;">₦${totalAmount.toLocaleString()}</p>
                  </td>
                  <td style="text-align: right;">
                    <p style="margin: 0 0 2px 0; font-size: 11px; color: #166534; font-weight: 600; text-transform: uppercase; letter-spacing: 0.1em;">Method</p>
                    <p style="margin: 0; font-size: 16px; font-weight: 600; color: #14532D; text-transform: capitalize;">${transaction?.channel || "Card"}</p>
                  </td>
                </tr>
              </table>
            </td>
          ...
          <tr>
            <td style="padding: 28px 32px;">
              ...
              <!-- Customer info -->
              <h2 style="margin: 0 0 12px 0; font-size: 13px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.1em; color: #6B7280;">
                Customer
              </h2>
              <table width="100%" cellpadding="0" cellspacing="0" style="background: #F9FAFB; border-radius: 8px; margin-bottom: 24px;">
                <tr>
                  <td style="padding: 14px 16px;">
                    <p style="margin: 0 0 4px 0; font-size: 15px; font-weight: 600; color: #111827;">${customerName}</p>
                    <p style="margin: 0 0 2px 0; font-size: 13px; color: #6B7280;">
                      <a href="mailto:${order.customer_email}" style="color: #14532D; text-decoration: none;">${order.customer_email}</a>
                    </p>
                    <p style="margin: 0; font-size: 13px; color: #6B7280;">
                      <a href="tel:${order.customer_phone}" style="color: #14532D; text-decoration: none;">${order.customer_phone}</a>
                    </p>
                  </td>
                </tr>
              </table>
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 20px; border: 1px solid #E5E7EB; border-radius: 8px; overflow: hidden;">
                <thead>
                  <tr style="background: #F3F4F6;">
                    <th style="padding: 10px 8px; font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.08em; color: #6B7280; text-align: left;">Product</th>
                    <th style="padding: 10px 8px; font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.08em; color: #6B7280; text-align: center;">Qty</th>
                    <th style="padding: 10px 8px; font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.08em; color: #6B7280; text-align: right;">Unit</th>
                    <th style="padding: 10px 8px; font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.08em; color: #6B7280; text-align: right;">Total</th>
                  </tr>
                </thead>
                <tbody>
                  ${itemsHTML}
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 28px;">
                <tr>
                  ...
                  <td style="padding: 5px 0; font-size: 13px; color: #111827; text-align: right; font-weight: 600;">₦${(order.subtotal || 0).toLocaleString()}</td>
                </tr>
                <tr>
                  <td style="padding: 5px 0; font-size: 13px; color: #6B7280;">Shipping</td>
                  <td style="padding: 5px 0; font-size: 13px; color: #111827; text-align: right; font-weight: 600;">${(order.shipping_cost || 0) === 0 ? "Free" : "₦" + (order.shipping_cost || 0).toLocaleString()}</td>
                </tr>
                ${
                  (order.transaction_fee || 0) > 0
                    ? `
                <tr>
                  <td style="padding: 5px 0; font-size: 13px; color: #6B7280;">Transaction Fee</td>
                  <td style="padding: 5px 0; font-size: 13px; color: #111827; text-align: right; font-weight: 600;">₦${(order.transaction_fee || 0).toLocaleString()}</td>
                </tr>`
                    : ""
                }
                <tr style="border-top: 2px solid #E5E7EB;">
                  <td style="padding: 12px 0 0 0; font-size: 16px; font-weight: 700; color: #111827;">Total Paid</td>
                  <td style="padding: 12px 0 0 0; font-size: 18px; font-weight: 700; color: #14532D; text-align: right;">₦${totalAmount.toLocaleString()}</td>
                </tr>
              </table>
              ...
              ${
                transaction?.paystack_reference
                  ? `
              <p style="margin: 0 0 28px 0; font-size: 12px; color: #9CA3AF;">
                Paystack Ref: <span style="font-family: monospace; color: #6B7280;">${transaction.paystack_reference}</span>
              </p>`
                  : ""
              }
              ...
              <div style="text-align: center;">
                <a href="${adminDashboardUrl}"
                   style="display: inline-block; background-color: #14532D; color: #FFFFFF; text-decoration: none; padding: 14px 28px; border-radius: 8px; font-size: 14px; font-weight: 600;">
          ...
          <tr>
            <td style="background: #F9FAFB; border-top: 1px solid #E5E7EB; padding: 16px 32px; text-align: center;">
              <p style="margin: 0; font-size: 12px; color: #9CA3AF;">
                This is an automated admin notification from Farz Supplements. Do not reply to this email.
              </p>
            </td>
          </tr>
